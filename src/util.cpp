...
const char * const BITCOIN_CONF_FILENAME = "blockdee.conf";
const char * const BITCOIN_PID_FILENAME = "blockdeed.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "blockdee" is a composite category enabling all BlockDee-related debug output
                if(ptrCategory->count(std::string("blockdee"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BlockDee
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BlockDee
    // Mac: ~/Library/Application Support/BlockDee
    // Unix: ~/.blockdee
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BlockDee";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BlockDee";
#else
    // Unix
    return pathRet / ".blockdee";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty blockdee.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override blockdee.conf
