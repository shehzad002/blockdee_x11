...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all BlockDee related functionality
...
/*
    ...
    allow endless transaction that would bloat BlockDee and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all BlockDee specific functionality
