...
// which are part of the BlockDee repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="blockdee-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /blockdee-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "blockdeepay-blockdee/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from blockdeepay-blockdee/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'blockdeepay-blockdee/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from blockdeepay-blockdee/develop"
            }
          }

          if (hasCache) {
            sh "cd /blockdee-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /blockdee-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /blockdee-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /blockdee-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /blockdee-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /blockdee-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /blockdee-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
