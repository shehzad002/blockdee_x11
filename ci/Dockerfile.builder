...
# blockdee_hash
RUN git clone https://github.com/blockdeepay/blockdee_hash
RUN cd blockdee_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} blockdee
RUN useradd -u ${USER_ID} -g blockdee -s /bin/bash -m -d /blockdee blockdee
...
RUN mkdir /blockdee-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /blockdee-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /blockdee-src
